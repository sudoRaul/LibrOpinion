...
async function onAvatarFile(event: Event) {
  ...
  if (err || !url) {
    error.value = t('editProfile.errAvatarUpload')
    return
  }
  avatarUrl.value = url
}
...
async function submit() {
  ...
  if (!USERNAME_RE.test(uname)) {
    error.value = t('editProfile.errUsernameFormat')
    return
  }
    ...
    >
      ...
      >
        <div class="mb-5 flex items-center justify-between">
          <h2 class="font-display text-2xl font-semibold text-stone-900 dark:text-white">{{ t('editProfile.title') }}</h2>
          <button
            type="button"
        ...
        <form class="space-y-4" @submit.prevent="submit">
          ...
          <div class="flex items-center gap-4">
            ...
            <div>
              ...
              >
                ...
                {{ uploading ? t('common.uploading') : t('editProfile.uploadImage') }}
                <input type="file" accept="image/*" class="hidden" :disabled="uploading" @change="onAvatarFile" />
              </label>
              <p class="mt-1 text-xs text-stone-400">{{ t('editProfile.avatarHint') }}</p>
            </div>
          </div>
          ...
          <div>
            <label for="bio" class="mb-1.5 block text-sm font-medium text-stone-700 dark:text-stone-300">
              {{ t('editProfile.bioLabel') }} <span class="font-normal text-stone-400">{{ t('common.optional') }}</span>
            </label>
            <textarea
              id="bio"
              v-model="bio"
          ...
          <AppTextField
            id="avatarUrl"
            v-model="avatarUrl"
            :label="t('editProfile.avatarUrlLabel')"
            type="url"
            :placeholder="t('editProfile.avatarUrlPlaceholder')"
            :hint="t('editProfile.avatarUrlHint')"
            :disabled="saving"
          />
          ...
          <div class="flex items-start justify-between gap-4 rounded-xl border border-stone-200 bg-stone-50 p-3.5 dark:border-stone-800 dark:bg-stone-800/40">
            <div class="min-w-0">
              <p class="text-sm font-medium text-stone-800 dark:text-stone-200">{{ t('editProfile.privateTitle') }}</p>
              <p class="mt-0.5 text-xs text-stone-500 dark:text-stone-400">
                {{ t('editProfile.privateDesc') }}
              </p>
            </div>
            <button
              type="button"
